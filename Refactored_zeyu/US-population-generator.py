...
# ... 前面的代码不变，直到 Main Execution Block ...

# 新增函数：封装人口生成并返回数据
def generate_population_data(location_name, core_cbg, min_cluster_pop):
    ...
    cbg_info = get_cbg_info(location_name, core_cbg, min_cluster_pop)
    ...
    sys.path.append(r'E:\JHU\Research\untitled folder\Algorithms\SP24\refactored\Output-zeyu')
    from papdata import Papdata
    papdata_processor = Papdata(prepared_data, place_path)
    
    # 生成并返回数据，而不是直接保存
    papdata_result = papdata_processor.generate(return_data=True)  # 假设 Papdata.generate() 支持返回数据
    return papdata_result

#if __name__ == "__main__":
    papdata_result = generate_population_data()
    print("Population data generated successfully.")
    # 可选：仍保存到文件供调试
    with open(r'E:\JHU\Research\untitled folder\Algorithms\SP24\refactored\papdata.json', 'w', encoding='utf-8') as f:
        json.dump(papdata_result, f)

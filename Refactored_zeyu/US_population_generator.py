...
import sys
import datetime
# Add path to import final_clustering_code_su23
sys.path.append(r'E:\JHU\Research\untitled folder\Algorithms\SP24\refactored')
from final_clustering import get_cbg_info
...
class Person:
    ...
    """
    def __init__(self, id, sex, age, cbg, household, hh_id, initial_time=None, tags: dict = None, work_naics=None):
        self.id = id
        self.sex = sex
        self.age = age
        self.cbg = cbg
        ...
        self.last_poi_time = initial_time  # 使用 initial_time 初始
    ...
    def need_new_poi(self, current_time: datetime.datetime) -> bool:
        if not self.availability:
            return False

        current_hour = current_time.hour
        work_start, work_end = self.work_time
        in_work_time = False
        if work_start <= work_end:
            in_work_time = work_start <= current_hour < work_end
        else:
            in_work_time = current_hour >= work_start or current_hour < work_end

        if self.at_home() and not in_work_time:
            return True

        if self.last_poi_time is None:
            return True
        time_diff = (current_time - self.last_poi_time).total_seconds() / 3600
        return time_diff >= 2
...
def enhance_households_with_movement_data(household_list, cbg_info):
    """
    Enhances household data with movement patterns from cbg_info
    """
    ...
    for group in household_list:
        for home in group:
            ...
            if cbg in cbg_lookup:
                ...
                # Adjust social tendencies based on movement patterns
...
def generate_population_data(core_cbg, min_cluster_pop):
    # Load population info
    with open(r'E:\JHU\Research\untitled folder\Algorithms\population_info.yaml', mode="r", encoding="utf-8") as file:
        pop_data = yaml.full_load(file)

    
    # Read census data
    census_df = pd.read_csv(r"E:\JHU\Research\untitled folder\Algorithms\safegraph_cbg_population_estimate.csv")
    
    # Get cbg_info directly from final_clustering_code_su23
    cbg_info = get_cbg_info(core_cbg, min_cluster_pop)
    ...
    places_df = pd.read_csv(place_path)
    ...
    # 创建 Papdata 实例
    papdata_processor = Papdata(hh_info=prepared_data, places_df=places_df)
    papdata_result = papdata_processor.generate()

    return papdata_result


#if __name__ == "__main__":
    result = generate_population_data( core_cbg='240430006012', min_cluster_pop=5000)
    print(result)
    print("Population data generated successfully.")
    print("Papdata JSON saved for reference.")